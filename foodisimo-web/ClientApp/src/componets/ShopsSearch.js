...
class ShopsSearch extends Component {
    ...
    componentDidMount() {
        fetch("https://localhost:5001/shop").then(res => res.json()).then(res => this.setState({shop_list: res}))
    }

    getShops() {
        let shop_list = this.state.shop_list;
        return shop_list.map((shop) => {
            console.log(shop)
            let categories = shop["categories"].map(s => s["name"])
            return (<div className="shops-search-shops">
                <div className="shops-search-shops-container">
                    <div className="shops-search-shops-icon">
                        <img alt="shop-logo" src={require('../images/'+shop["id"]+'.png')} />
                    </div>
                    <div className="shops-search-shops-info">
                        <h3>{shop["name"]}</h3>
                        <small>
                            <span>{shop["address"]}</span>
                            <span>{shop["distance"]}χλμ</span>
                        </small>
                    </div>
                    ...
                    <div className="shops-search-shops-button">
                        <div>
                            <Link to={"/shops/menu?id=" + shop["id"]} className="offers-info">Παράγγειλε</Link>
                        </div>
                    </div>
                </div>
            </div>)
